...
def create_user(_admin_user):
    ...
    name_str = LanguageString(
        id=str(uuid.uuid4()), content_by_language={language: params["name"]}
    )
...
@admin_api.route("/register_clinic", methods=["POST"])
@admin_authenticated
def register_clinic(_admin_user):
    params = assert_data_has_keys(request, {"name"})
    clinic = Clinic(
        id=str(uuid.uuid4()),
        edited_at=datetime.now(),
        name=LanguageString(id=str(uuid.uuid4()), content_by_language={"en": "EMA"}),
    )
