...
class Patient(ClientObject):
    ...
    def client_insert_values(self):
        return [
            ...
            self.format_string(self.section),
            self.serial_number,
            self.registered_by_provider_id,
            self.phone,
    ...
    def client_update_sql(cls):
        return """UPDATE patients SET given_name = ?, surname = ?, date_of_birth = ?, sex = ?, country = ?, hometown = ?, section = ?, serial_number = ?, registered_by_provider_id = ?, phone = ?, edited_at = ? WHERE id = ?"""
    ...
    def server_insert_sql(cls):
        return """INSERT INTO patients (id, given_name, surname, date_of_birth, sex, country, hometown, section, serial_number, registered_by_provider_id, phone, edited_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
    ...
    def server_update_sql(cls):
        return """UPDATE patients SET given_name = %s, surname = %s, date_of_birth = %s, sex = %s, country = %s, hometown = %s, section = %s, serial_number = %s, registered_by_provider_id = %s, phone = %s, edited_at = %s WHERE id = %s"""

    @classmethod
    def db_columns_from_server(cls):
        return [
            ("id", lambda s: s.replace("-", "")),
            ...
            ("section", cls.make_language_string),
            ("serial_number", identity),
            ("registered_by_provider_id", identity),
            ("phone", identity),
            ("edited_at", identity),
        ]
    ...
    def from_db_row(cls, db_row):
        ...
        return cls(
            id,
            LanguageString.from_id(given_name),
            LanguageString.from_id(surname),
            date_of_birth,
            sex,
            LanguageString.from_id(country),
            LanguageString.from_id(hometown),
            LanguageString.from_id(section),
            serial_number,
            registered_by_provider_id,
            phone,
            edited_at,
    ...
    def to_dict(self):
        return {
            ...
            "registered_by_provider_id": self.registered_by_provider_id,
            "phone": self.phone,
